...
6. 服务器旧代码备份路径：`/home/ubuntu/xiaoluoke-server-backups/xiaoluoke-before-deploy-20260506-224824.tar.gz`
7. `.env.production` 备份路径：`/home/ubuntu/xiaoluoke-server-backups/.env.production.backup.20260506-224824`
8. `.env.production` 已保留为服务器项目目录中的未跟踪文件。
9. 部署结果：`git reset --hard origin/main` 成功，`src/app/admin/login/page.tsx` 存在，服务器 `npm run build` 通过，生成 78/78 页面/API 路由，PM2 `xiaoluoke-web` online。
10. 公网测试：`http://43.134.164.29/customer/home`、`http://43.134.164.29/worker/login`、`http://43.134.164.29/admin/login` 均返回 200 OK。
11. 服务器本机测试：`http://127.0.0.1:3000/customer/home`、`http://127.0.0.1:3000/worker/login`、`http://127.0.0.1:3000/admin/login` 均返回 200 OK。
12. 当前测试网仍是 localStorage/mock MVP，不同设备不会真正共享订单、钱包、聊天等业务数据。
