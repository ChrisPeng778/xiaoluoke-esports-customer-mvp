...
当前优先模块状态：设置模块统一数据源已完成；会员等级配置并入 shared store 已完成；投诉 / 反馈 / 售后 / 评价闭环已完成；管理端权限与危险操作一致性已完成；订单/钱包/退款/状态一致性已完成。最近一次 `npm run build` 已通过，生成 78 个页面/API 路由。

下一轮建议事项：不要直接开发新大功能，建议做“小问题清理与上线前检查”，包括 `/customer/must-read` 同步设置、旧提现页面文案统一、接单员钱包流水正负号显示、全站“打手”文案检查。不要顺手修真实退款到账、真实提现、真实支付、短信、企业微信、COS/OSS 或 MongoDB；不要重做设置、会员等级、投诉/反馈/售后/评价闭环、权限结构或订单状态机。
