...
| shared store / localStorage | 部分完成 | `src/lib/store.ts` 和 `src/lib/types.ts` 已包含主 store、session、三端大量业务函数；`system_settings` 已并入 `xiaoluoke_customer_mvp_store`。 | 会员等级配置仍使用独立 key `xiaoluoke_admin_member_level_settings`，下一轮建议迁入 shared store。 |
